# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""
# count=$(echo "$(bitcoin-cli -regtest getblockcount) + 2016" | bc)
count=2041 #25 + difficulty adjustment interval of 2016 blocks
addy=2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP
amount=0.2
tx=$(bitcoin-cli -regtest decoderawtransaction )
utxo_txid=$(echo $tx | jq -r '.txid')
rawtxhex=$(bitcoin-cli -regtest -named createrawtransaction inputs='''[ { "txid": "'$utxo_txid'", "vout": 0 }, { "txid": "'$utxo_txid'", "vout": 1 } ]''' outputs='''{ "'$addy'": 0.2 }''' locktime="$count")
echo $rawtxhex
